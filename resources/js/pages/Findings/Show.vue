...
const datacenterId = computed(
    () => props.finding.audit?.datacenter?.id ?? null,
);
...
const updatesWithConflicts = computed<RealtimeUpdate[]>(() => {
    return pendingUpdates.value.map((update) => ({
        ...update,
        isConflict:
            update.entityType === 'finding' &&
            update.entityId === props.finding.id,
    }));
});
...
const handleQuickTransition = async (
    targetStatus: FindingStatusValue,
    notes?: string,
) => {
...
const isAdmin = computed(() => {
    ...
    return props.quickActions.some(
        (action) =>
            action.action === 'reopen' && props.finding.status === 'resolved',
    );
});
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
            ...
            <div
                v-if="
                    timeMetrics.time_to_first_response ||
                    timeMetrics.total_resolution_time
                "
            ...
            >
                <Card v-if="timeMetrics.time_to_first_response">
                    <CardContent class="flex items-center gap-3 p-4">
                        <div
                            class="flex size-10 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900/30"
                        >
                            <Timer
                ...
                <Card v-if="timeMetrics.total_resolution_time">
                    <CardContent class="flex items-center gap-3 p-4">
                        <div
                            class="flex size-10 items-center justify-center rounded-full bg-green-100 dark:bg-green-900/30"
                        >
                            <CheckCircle
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <Card>
                    ...
                    <CardContent class="space-y-4">
                        ...
                        <div
                            v-if="finding.discrepancy_type_label"
                            class="grid gap-2"
                        >
                            <dt
                                class="text-sm font-medium text-muted-foreground"
                            ...
                            <dd class="text-sm">
                                {{ finding.discrepancy_type_label }}
                            </dd>
                        </div>
                        <div v-if="finding.due_date" class="grid gap-2">
                            ...
                            <dd class="flex items-center gap-2 text-sm">
                                ...
                                {{ finding.due_date }}
                                <span
                                    v-if="finding.is_overdue"
                                    class="text-red-600 dark:text-red-400"
                                    >(Overdue)</span
                                >
                                <span
                                    v-else-if="finding.is_due_soon"
                        ...
                        <form @submit.prevent="submitForm" class="space-y-6">
                            <div class="grid gap-6 md:grid-cols-2">
                                ...
                                <div class="space-y-2">
                                    ...
                                    />
                                    <p
                                        v-if="form.errors.finding_category_id"
                                        class="text-xs text-destructive"
                                    >
                                        {{ form.errors.finding_category_id }}
                                    </p>
                                </div>
                                ...
                                <div class="flex justify-between text-xs">
                                    <div>
                                        <p
                                            ...
                                            class="text-amber-600 dark:text-amber-400"
                                        >
                                            Minimum 10 characters required when
                                            resolving.
                                        </p>
                                        <p
                                            v-else-if="resolutionNotesRequired"
                                        ...
                                        <p
                                            v-if="form.errors.resolution_notes"
                                            class="text-destructive"
                                        >
                                            {{ form.errors.resolution_notes }}
                                        </p>
                                    ...
                                    <span
                                        :class="
                                            resolutionNotesRequired &&
                                            resolutionNotesCharCount < 10
                                                ? 'text-amber-600'
                                                : 'text-muted-foreground'
                                        "
                                    >
                                        {{ resolutionNotesCharCount }}
                                        characters
                                    </span>
                                </div>
                            </div>
