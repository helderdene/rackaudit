...
import { index, destroy, toggle, history } from '@/actions/App/Http/Controllers/ReportScheduleController';
...
const needsReEnable = computed(() => {
    return !props.schedule.is_enabled && props.schedule.consecutive_failures >= 3;
});
...
const handleToggle = (newValue: boolean) => {
    ...
    router.patch(toggle.url(props.schedule.id), {
        is_enabled: newValue,
    }, {
        preserveScroll: true,
        onFinish: () => {
        ...
        <div class="flex h-full flex-1 flex-col gap-6 rounded-xl p-4">
            ...
            <Alert v-if="needsReEnable" variant="destructive">
                ...
                <AlertDescription class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <span>
                        This schedule was disabled after {{ schedule.consecutive_failures }} consecutive failures.
                        Re-enabling will reset the failure count and calculate the next run time.
                    </span>
                    <Button
                        v-if="canToggle"
                        variant="outline"
                    ...
                    >
                        <RefreshCw class="mr-2 h-4 w-4" :class="{ 'animate-spin': isToggling }" />
                        Re-enable Schedule
                    </Button>
                </AlertDescription>
            </Alert>
                ...
                <div class="space-y-6 lg:col-span-2">
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                The report settings captured when this schedule was created.
                            </CardDescription>
                        </CardHeader>
                        <CardContent class="space-y-4">
                            <div class="grid gap-4 sm:grid-cols-2">
                                <div>
                                    <dt class="text-sm font-medium text-muted-foreground">Report Type</dt>
                                    <dd class="mt-1">
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1">
                                        <Badge :variant="schedule.format === 'pdf' ? 'default' : 'secondary'">
                            ...
                            <div>
                                <dt class="text-sm font-medium text-muted-foreground">Columns</dt>
                                <dd class="mt-1 text-sm">{{ columnsList }}</dd>
                            </div>

                            <div v-if="hasFilters">
                                <dt class="text-sm font-medium text-muted-foreground">Filters</dt>
                                <dd class="mt-1">
                                    <div class="flex flex-wrap gap-2">
                                        <Badge v-for="filter in formattedFilters" :key="filter.key" variant="secondary">
                            ...
                            <div v-if="hasSort">
                                <dt class="text-sm font-medium text-muted-foreground">Sort</dt>
                                <dd class="mt-1 text-sm">{{ formattedSort }}</dd>
                            </div>

                            <div v-if="schedule.report_configuration.group_by">
                                <dt class="text-sm font-medium text-muted-foreground">Group By</dt>
                                <dd class="mt-1 text-sm">{{ schedule.report_configuration.group_by }}</dd>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Recent report generation attempts and their outcomes.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                        ...
                        <CardContent>
                            <div class="flex items-center justify-between">
                                <Label for="schedule-toggle" class="text-sm">
                                    {{ schedule.is_enabled ? 'Schedule is enabled' : 'Schedule is disabled' }}
                                </Label>
                                <Switch
                                    id="schedule-toggle"
                    ...
                    <Card>
                        ...
                        <CardContent class="space-y-4">
                            <div class="flex items-center gap-2 text-sm">
                                ...
                                <span class="font-medium">{{ schedule.distribution_list.name }}</span>
                            </div>
                            <p class="text-sm text-muted-foreground">
                                {{ schedule.distribution_list.members_count }}
                                {{ schedule.distribution_list.members_count === 1 ? 'recipient' : 'recipients' }}
                            </p>

                            <!-- Show member emails if loaded -->
                            <div v-if="schedule.distribution_list.members?.length" class="space-y-1">
                                <Separator />
                                <ul class="mt-2 space-y-1 text-sm text-muted-foreground">
                                    <li
                                        v-for="member in schedule.distribution_list.members"
                                        :key="member.id"
            ...
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        ...
                        Future scheduled runs will be cancelled and execution history will be lost.
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
