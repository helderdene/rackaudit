...
const canResolve = computed(() => {
    return (
        props.discrepancy.status === 'open' ||
        props.discrepancy.status === 'acknowledged'
    );
});
...
const hasMismatchDetails = computed(() => {
    return (
        props.discrepancy.mismatch_details &&
        Object.keys(props.discrepancy.mismatch_details).length > 0
    );
});
...
// Format date
const getMismatchField = (value: unknown, field: string): string => {
...
const acknowledge = async () => {
    ...
    try {
        const response = await fetch(
            `/api/discrepancies/${props.discrepancy.id}/acknowledge`,
            {
                method: 'PATCH',
                headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(
                data.message || 'Failed to acknowledge discrepancy',
            );
        }
...
const resolve = async () => {
    ...
    try {
        const response = await fetch(
            `/api/discrepancies/${props.discrepancy.id}/resolve`,
            {
                method: 'PATCH',
                headers: {
                    'Content-Type': 'application/json',
            ...
            <div class="space-y-6">
                ...
                <div class="flex flex-wrap items-center gap-4 text-sm">
                    <div class="flex items-center gap-2">
                        ...
                        <Badge
                            :variant="getStatusBadgeVariant(discrepancy.status)"
                        >
                            {{ discrepancy.status_label }}
                        </Badge>
                    </div>
                    <div
                        v-if="discrepancy.datacenter"
                    ...
                    <div
                        v-if="discrepancy.room"
                        class="flex items-center gap-2"
                    >
                        <span class="text-muted-foreground">Room:</span>
                        <span class="font-medium">{{
                            discrepancy.room.name
                        }}</span>
                    </div>
                </div>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <Card>
                        ...
                        <CardContent
                            v-if="formatPortDisplay(discrepancy.source_port)"
                            class="space-y-2 text-sm"
                        >
                            <div class="grid grid-cols-2 gap-1">
                                <span class="text-muted-foreground">Port:</span>
                                <span class="font-medium">{{
                                    formatPortDisplay(discrepancy.source_port)
                                        ?.port
                                }}</span>
                            </div>
                            <div class="grid grid-cols-2 gap-1">
                    ...
                    <Card class="md:col-start-2">
                        ...
                        <CardContent
                            v-if="formatPortDisplay(discrepancy.dest_port)"
                            class="space-y-2 text-sm"
                        >
                            <div class="grid grid-cols-2 gap-1">
                                <span class="text-muted-foreground">Port:</span>
                                <span class="font-medium">{{
                                    formatPortDisplay(discrepancy.dest_port)
                                        ?.port
                                }}</span>
                            </div>
                            <div class="grid grid-cols-2 gap-1">
                    ...
                    <CardContent>
                        <div class="space-y-3">
                            <div
                                v-for="(
                                    value, key
                                ) in discrepancy.mismatch_details"
                                :key="String(key)"
                ...
                <div
                    v-if="
                        discrepancy.expected_config || discrepancy.actual_config
                    "
