...
import { AlertTriangle, CheckCircle, Server, XCircle } from 'lucide-vue-next';
...
const deviceDetails = computed(() => {
    ...
    return {
        ...
        position: formatUPosition(
            props.verification.device.start_u,
            props.verification.device.u_height,
        ),
    };
});

const showNotesRequired = computed(
    () => action.value === 'not_found' || action.value === 'discrepant',
);
...
function handleSubmit(): void {
    ...
    if (showNotesRequired.value && !notes.value.trim()) {
        notesError.value =
            'Notes are required when marking device as not found or discrepant.';
        return;
    }
            ...
            <div class="space-y-4 py-4">
                ...
                >
                    <div class="flex items-start gap-3">
                        ...
                        <div class="space-y-1">
                            ...
                            <div
                                v-if="deviceDetails.assetTag"
                                class="text-muted-foreground"
                            >
                                Asset Tag: {{ deviceDetails.assetTag }}
                            </div>
                            <div
                                v-if="deviceDetails.serialNumber"
                                class="text-muted-foreground"
                            >
                            ...
                            <div
                                v-if="deviceDetails.rack"
                                class="text-muted-foreground"
                            >
                                Location: {{ deviceDetails.rack }} @
                ...
                <div class="space-y-2">
                    <Label for="notes" class="text-sm font-medium">
                        Notes
                        <span v-if="showNotesRequired" class="text-red-500"
                            >*</span
                        >
